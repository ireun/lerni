...
def account_base(request):
    ...
    page.update({'first_name': user.first_name, 'second_name': user.second_name, 'last_name': user.last_name,
                 'pesel': user.pesel, 'birthdate': str(user.birth_date),
                 'gender': [u"Kobieta", u"Mężczyzna"][user.is_male],
...
def account_phone_app(request):
    ...
    try:
        ...
        if logged_in:
            ...
            app_codes = DBSession.query(AppCodes).filter_by(user_id=user_id)
            if 'action' in request.params:
                if request.params['action'] == "add_phone":
                    if app_codes.count() > 9:
                        page["allerts"].append([u"Limit podłączonych urządzeń (10) został osiągnięty.",
                                                "error", "topRight"])
                    elif app_codes.filter_by(phone_id="phone_id").count() > 0:
                        page["allerts"].append([u"Wykorzystaj wygenerowany kod QR do"
                                                u"podłączenia urządzenia,"
                                                u"a następnie wygeneruj kolejny.", "allert", "topRight"])
                    else:
                        with transaction.manager:
                            model = AppCodes(user_id, "phone_name", "phone_id",
                                             hashlib.sha224(str(random.random())).hexdigest())
                            DBSession.add(model)
                elif request.params['action'] == "delete":
                    if 'code' in request.params:
                        with transaction.manager:
                            DBSession.query(AppCodes).filter_by(code=request.params['code']).delete()
            page['phones'] = []
            for position in DBSession.query(AppCodes).filter_by(user_id=user_id):
                page['phones'].append([position.phone_name, position.code, position.phone_id])
        else:
            #dodać redirecta na stronę logowania
            return Response("User not logged in", content_type='text/plain', status_int=500)
    except DBAPIError:
        return Response("Mysql connection error", content_type='text/plain', status_int=500)
    return page
...
def account_folders(request):
    page = {'editor': 0, 'breadcrumbs': [["/admin/overview", u"Dashboard"], ["", u"Moje konto"], ["", u"Foldery"]],
            'allerts': []}
...
def account_entries(request):
    page = {'editor': 0, 'breadcrumbs': [["/admin/overview", u"Dashboard"], ["", u"Moje konto"], ["", u"Wpisy"]],
            'allerts': []}
    page.update(get_basic_account_info(request))
    logged_in = authenticated_userid(request)
    page['name'] = username(logged_in)
...
def account_presentations(request):
    page = {'editor': 0, 'breadcrumbs': [["/admin/overview", u"Dashboard"], ["", u"Moje konto"], ["", u"Prezentacje"]],
            'allerts': []}
    page.update(get_basic_account_info(request))
    logged_in = authenticated_userid(request)
    page['name'] = username(logged_in)
...
def account_tasks_sets(request):
    ...
    #def account_tasks_sets(request):
    #    page={'editor':0, 'breadcrumbs':[["/admin/overview",u"Dashboard"],
    # ["",u"Moje konto"],["",u"Konkursy"]], 'allerts':[]}
    #    page.update(get_basic_account_info(request))
    #    logged_in = authenticated_userid(request)
    #    page['name']=username(logged_in)
    #    page['title']=u"Foldery"
